...
void check_potential_moves(
    char piece);  // checks if the potential moves are correct
...
bool check_endgame() {
  ...
  if (is_checked(king_row, king_col)) {
    ...
    if (Board[king_row][king_col] == 'k') {  // whites turn
      ...
      check_potential_moves('k');
      for (int i = 0; i < 8; i++) {
        for (int j = 0; j < 8; j++) {
          if (potential_moves_board[i][j] == 'x')
            return true;  // checking to see if the king has any place to go
        }
      }
...
void potential_moves(char piece, int row, int col) {
  ...
  switch (piece) {
     ...
     * ***********************************************************/
    case 'p':
      if (is_check_blocker('p', move[2],
                           move[1])) {  // if the piece is a check blocker
      }
      ...
      for (int i = -1; i < 2 && (row + i) >= 0 && (row + i) < 8; i++) {
        for (int j = -1; j < 2 && (col + j) >= 0 && (col + j) < 8; j++) {
          if ((Board[row + i][col + j] == 'o' || Board[row + i][col + j] < Z) &&
              !is_check_blocker('p', row + i, col + j))
...
void piece_is_check_blocker(char piece, int dy, int dx, int row, int col) {
  int checky = checking_piece_row - row;
  int checkx = checking_piece_col - col;

  if (checky != 0)
    checky = (checky < 0) ? -1 : 1;
  if (checkx != 0)
    checkx = (checkx < 0) ? -1 : 1;
  if (dx != checkx || dy != checky) return;
...
void check_potential_moves(
    char piece) {  // call this only if the player is checked
  if (piece == 'k' || piece == 'K')
...
bool is_check_blocker(char piece, int row, int col) {
  Board[row][col] = 'o';  // remove the piece to see if its still checked
