  ...
  switch (piece) {
    ...
    case 'p':
      if (Board[row - 1][col] == 'o' &&
          !is_check_blocker(row, col)) {  // if there is no piece in front
    ...
    case 'r':
      ...
      for (int i = col + 1; (i < 8 && !is_check_blocker(row, col)); i++) {
        if (Board[row][i] == 'o') {
          ...
          if (col == 0 && Board[row][i] == 'k' && !lw_rook_moved &&
              !white_king_moved) {
      ...
      for (int i = col - 1; (i >= 0 && !is_check_blocker(row, col)); i--) {
        if (Board[row][i] == 'o') {
          ...
          if (col == 7 && Board[row][i] == 'k' && !rw_rook_moved &&
              !white_king_moved) {
            potential_moves_board[row][i + 1] = 'x';
            castling_enable = true;
          }
    ...
    case 'R':
      ...
      for (int i = col + 1; (i < 8 && !is_check_blocker(row, col)); i++) {
        if (Board[row][i] == 'o') {
          ...
          if (col == 0 && Board[row][i] == 'K' && !lb_rook_moved &&
              !black_king_moved) {
            potential_moves_board[row][i - 1] = 'x';
            castling_enable = true;
          }
      ...
      for (int i = col - 1; (i >= 0 && !is_check_blocker(row, col)); i--) {
        if (Board[row][i] == 'o') {
          ...
          if (col == 7 && Board[row][i] == 'k' && !rb_rook_moved &&
              !black_king_moved) {
            potential_moves_board[row][i + 1] = 'x';
            castling_enable = true;
          }
    ...
    case 'b':
      // up right
      for (int i = 1; row + i < 8 && col + i < 8 && !is_check_blocker(row, col);
           i++) {
      ...
      // up left
      for (int i = 1;
           row + i < 8 && col - i >= 0 && !is_check_blocker(row, col); i++) {
        if (Board[row + i][col - i] == 'o') {
          potential_moves_board[row + i][col - i] = 'x';
        } else if (Board[row + i][col - i] >= 'a' &&
                   Board[row + i][col - i] <= 'z') {
      ...
      for (int i = 1;
           row - i >= 0 && col + i < 8 && !is_check_blocker(row, col); i++) {
        if (Board[row - i][col + i] == 'o') {
          potential_moves_board[row - i][col + i] = 'x';
        } else if (Board[row - i][col + i] >= 'a' &&
                   Board[row - i][col + i] <= 'z') {
      ...
      for (int i = 1;
           row - i >= 0 && col - i >= 0 && !is_check_blocker(row, col); i++) {
        if (Board[row - i][col - i] == 'o') {
          potential_moves_board[row - i][col - i] = 'x';
        } else if (Board[row - i][col - i] >= 'a' &&
                   Board[row - i][col - i] <= 'z') {
...
void check_potential_moves() { // call this only if the player is checked
  int row = 0;
  int col = 0;
  for (int i = 0; i < 8; i++) {
    for (int j = 0; j < 8; j++) {
      if (turn == 'b' && Board[i][j] == 'k') {
        // found the white king
        row = i;
        col = j;
        goto found_king;
      } else if (turn == 'w' && Board[i][j] == 'K') {
        // found the black king
        row = i;
        col = j;
        goto found_king;
      }
  ...
  found_king:
    for (int i = 0; i < 8; i++) {
        for (int j = 0; j < 8; j++) {
            if (potential_moves_board[i][j] == 'x' && is_check_blocker(i, j))
                continue;
            else {
...
bool is_check_blocker(int row, int col) {  // take location of piece
        ...
        if (Board[i][j] == 'k' && Board[row][col] > 97) {
          ...
          if (dx == 0 || dy == 0 || dx == dy || dx == -dy) {
            ...
            for (int k = 0;
              ...
              if ((dx == dy || dx == -dy) &&
                  (Board[posy][posx] == 'B' ||
                   Board[posy][posx] == 'Q')) {  // bishop or queen
                return true;
              } else if ((dx == 0 || dy == 0) && (Board[posy][posx] == 'R' ||
                                                  Board[posy][posx] == 'Q')) {
                return true;
              }
            }
...
bool is_checked(int row, int col) {  // take location of king
  for (int i = -1; i < 2; i++) {
    for (int j = -1; j < 2; j++) {
      for (int k = 1; j < 8; k++) {
        int posy = row + (k * i);
        ...
        if (posy < 0 || posy > 7 || posx < 0 || posx > 7)
          break;  // stop proceeding if we are out of bounds
